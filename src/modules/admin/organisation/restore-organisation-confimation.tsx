...
export const RestoreOrganisationConfirmation = ({ isOpen, onClose, organisationID, onSuccess }: any) => {
    const { user } = useSession();
    const [organisation, setOrganisation] = useState<Organisation | null>(null);

    if (!isOpen) return null;

    const restoreOrganisation = async () => {
        try{
            const url = `organisation/restore-organisation/${organisationID}`
            ...
            toast.error('Organisation Not Restored', {
        ...
        <div className="h-screen fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="relative bg-white rounded-lg shadow-lg p-6 w-[450px]">
                ...
                <div>
                    ...
                    <p className="text-gray-600 pb-4">Are you sure you want to restore this organisation? This action cannot be undone.</p>
                </div>
                <div className="flex justify-end space-x-2">
                    ...
                    <Button onClick={() => restoreOrganisation() } className="bg-red text-white border-red hover:bg-rose-300 hover:border-rose-300 h-8">
                        Confirm
                    </Button>
                </div>
            </div>
