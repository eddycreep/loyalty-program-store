...
export const DeactivateOrganisationConfirmation = ({ isOpen, onClose, organisationID, onSuccess }: any) => {
    const { user } = useSession();
    const [organisation, setOrganisation] = useState<Organisation | null>(null);

    if (!isOpen) return null;

    const deactivateOrganisation = async () => {
        try{
            const url = `organisation/deactivate-organisation/${organisationID}`
            ...
            toast.error('Organisation Not Deactivated', {
    ...
    return (
        <div className="h-screen fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="relative bg-white rounded-lg shadow-lg p-6 w-[450px]">
                ...
                <div>
                    <div className="text-lg font-bold pb-2 text-gray-600">Confirm Deactivation</div>
                    <p className="text-gray-600 pb-4">Are you sure you want to deactivate this organisation? This action cannot be undone.</p>
                </div>
                <div className="flex justify-end space-x-2">
                    ...
                    <Button onClick={() => deactivateOrganisation() } className="bg-red text-white border-red hover:bg-rose-300 hover:border-rose-300 h-8">
                        Confirm
                    </Button>
                </div>
            </div>
