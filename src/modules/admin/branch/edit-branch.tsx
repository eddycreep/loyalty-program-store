...
import { Branch, CreateBranch, SuccessResponse } from '@/modules/types/branch/branches-types';
...
export function EditBranch({ onClose, selectedBranch, onSuccess }: any) {
    ...
    // Fixed useEffect to properly populate form with selected branch data
    useEffect(() => {
        if (selectedBranch && selectedBranch.uid) {
            // Parse operatingHours from JSON if it exists, otherwise use default structure
            ...
            if (selectedBranch.operatingHours) {
                try {
                    // Handle both string and object formats for operatingHours
                    const operatingHoursData = typeof selectedBranch.operatingHours === 'string' 
                        ? JSON.parse(selectedBranch.operatingHours) 
                        : selectedBranch.operatingHours;
                    
                    parsedOperatingHours = {
                        days: operatingHoursData.days || [],
                        opening: operatingHoursData.opening || '',
                        closing: operatingHoursData.closing || ''
                    };
                } catch (error) {
